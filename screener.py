...
def _download_daily_closes(
    ...
    period: str = "8mo",
    pause_sec: float = 0.5,
    fail_streak_limit: int = 30,
    cooldown_sec: int = 120,
    max_cooldowns: int = 6,
) -> dict[str, pd.Series]:
    """逐檔序列下載日 K 收盤價，回傳 {ticker: close series}。

    永不使用 yf.download 批次。實跑結論（2026-07-19 Actions ×3、
    2026-09-17 本機住宅 IP）：批次 burst 一下去 Yahoo 就把整個 IP 的額度
    打爆，之後連序列請求都全數 429，恢復以小時到天計——與 IP 類型無關。
    相對地，逐檔 Ticker.history 在限流日只是慢、不會死。

    本機 launchd 沒有 job timeout，慢一點沒關係：
    - 每檔一次請求，檔間 pause_sec 喘息（2000+ 檔約 60～70 分鐘）
    - 連續 fail_streak_limit 次失敗（含靜默回空）= 撞到限流 → 冷卻
      cooldown_sec 再繼續；冷卻 max_cooldowns 次用完仍全滅才提早斷
    - 第一輪失敗的 ticker 做一輪補抓（限流視窗內掉的檔多半能補回）
    - 覆蓋率 < 50% 直接 raise 讓 job 紅掉，不產出殘缺結果
    """
    closes: dict[str, pd.Series] = {}
    total = len(tickers)
    error_sample: dict[str, str] = {}
    state = {"cooldowns_used": 0, "empty": 0}

    def _serial_pass(pending: list[str], label: str) -> tuple[list[str], bool]:
        """回傳 (失敗清單, 是否因冷卻用盡而提早斷)"""
        failed: list[str] = []
        fail_streak = 0
        for i, ticker in enumerate(pending, 1):
            ...
            try:
                ...
                series = (
                    df["Close"].dropna() if "Close" in df else pd.Series(dtype=float)
                )
                if not series.empty:
            ...
            if ok:
                ...
                if fail_streak >= fail_streak_limit:
                    if state["cooldowns_used"] >= max_cooldowns:
                        logger.error(
                            {
                                "message": "Cooldowns exhausted and still failing, "
                                "aborting early",
                                "pass": label,
                                "at_ticker": i,
                        ...
                        return failed, True
                    state["cooldowns_used"] += 1
                    logger.warning(
                        {
                            "message": "Sustained failures — rate-limited, cooling down",
                            "pass": label,
                            "at_ticker": i,
                            "cooldown_sec": cooldown_sec,
                            "cooldowns_left": max_cooldowns - state["cooldowns_used"],
                        }
                    )
            ...
            if i % 200 == 0:
                logger.info(f"[{label} {i}/{len(pending)}] {len(closes)} closes so far")
            time.sleep(pause_sec)
        return failed, False

    failed, aborted = _serial_pass(list(tickers), "serial")
    logger.info(
        f"[{len(closes)}/{total}] daily closes after first pass"
        f" (failed {len(failed)}, empty {state['empty']})"
    )
    if failed and not aborted:
        logger.info(f"Retrying {len(failed)} failed tickers after cooldown...")
        time.sleep(cooldown_sec)
        failed, _ = _serial_pass(failed, "retry")
        logger.info(
            f"[{len(closes)}/{total}] daily closes after retry pass"
